...
class Pie extends React.Component {
  ...
  constructor(props: Props) {
    ...
    this._ratio = this._ratio.bind(this);
  ...
  _ratio(item, total) { return parseFloat(item.number > 0 ? item.number / total * 100 : 0).toFixed(2); }
        ...
        <View style={{position: 'absolute', top:margin, left: 2*margin + this.props.pieWidth}}>
          {
            this.props.data.map( (item, index) =>
            {
              ...
              return (
                  <View key={index}>
                    <Text style={[styles.label, {color: this._color(index), fontWeight: 'normal'}]}>
                      {this._label(item)}: {this._value(item)} ({this._ratio(item, total)}%)</Text>
                  </View>
              );
            })
          }
